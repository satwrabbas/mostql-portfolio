...
import { FaGlobe, FaVolumeMute, FaSun, FaMoon, FaPalette } from "react-icons/fa";
...
export default function Header() {
  ...
  const { isDarkMode, toggleDarkMode, cycleColorPalette, colorPalette } = useTheme();
  ...
  // دالة لتحديد لون أيقونة الباليت بناءً على الثيم الحالي
  const getPaletteIconColor = (palette: ColorPalette) => {
    if (isDarkMode) {
      switch (palette) {
        case "slate": return "text-sky-400";
        case "stone": return "text-orange-400";
        default: return "text-zinc-300"; // For "zinc"
      }
    } else {
      switch (palette) {
        case "slate": return "text-sky-700";
        case "stone": return "text-orange-700";
        default: return "text-zinc-600"; // For "zinc"
      }
    }
  };
    ...
    <header className="fixed top-0 w-full z-50 py-4 md:py-6 transition-all duration-500">
      <div className="max-w-6xl mx-auto px-4 md:px-6 flex justify-between items-center">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          <div className={`relative w-9 h-9 md:w-12 md:h-12 rounded-xl overflow-hidden flex items-center justify-center border transition-colors duration-500
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          {/* 2. Palette Cycle Button (Modified) */}
          <button
            ...
            className={`flex items-center justify-center w-10 h-10 rounded-full border transition-all duration-300 relative group
             ...
             ${paletteIconColor} /* تطبيق اللون الديناميكي هنا */
            `}
          >
            <FaPalette size={16} />
